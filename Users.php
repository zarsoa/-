...
class Users extends Model
{
    ...
    protected $info = [
                    'tel.require'       => 'Email cannot be empty!',
                    'tel.mobile'        => 'Mailbox format error!',
                    'username.length'   => 'Usernames are 3-10 characters long!',
                    'username.require'  => 'The username cannot be empty!',
     ...
     * @return array
     */
    public function add_users($tel,$user_name,$pwd,$parent_id,$task_pddnum,$task_jdnum,$task_tbnum,$task_tmnum,$task_wxnum,$task_dznum,$ip,$head_pic,$xiaoxi,$pwd2='')
    {
        $tmp = Db::table($this->table)->where(['tel'=>$tel])->count();
        if($tmp){
            return ['code'=>1,'info'=>lang('邮箱已注册')];
        }
        if($ip != ''){
            // $tmp = Db::table($this->table)->where(['ip'=>$ip])->count();
            // if($tmp > 5){
            //     return ['code'=>1,'info'=>lang('您的IP注册人数已达上限，请联系在线客服！')];
            // }
        }
        ...
        $pt = Db::table('xy_level')->where('level',0)->value('yx_time');
        $yx_time = $pt*86400 + time();
        ...
        $data = [
            ...
            'pwd'           => $pwd,
            'parent_id'     => $parent_id,
            'task_pddnum'      => $task_pddnum,
            'task_jdnum'      => $task_jdnum,
            'task_tbnum'      => $task_tbnum,
            'task_tmnum'      => $task_tmnum,
            'task_wxnum'      => $task_wxnum,
            'task_dznum'      => $task_dznum,
            'ip'      => $ip,
            'headpic'      => $head_pic,
            'xiaoxi'      => $xiaoxi,
            'viptime'      => 0
        ...
        if($parent_id){
            $res2 = Db::table($this->table)->where('id',$data['parent_id'])->update(['childs'=>Db::raw('childs+1')]);
        }else{
            $res2 = true;
        }
     ...
     * @return array
     */
    public function edit_users($id,$tel,$user_name,$pwd,$parent_id,$balance,$freeze_balance,$token,$pwd2='', $topat){
    ...
    public static function create_qrcode($invite_code,$user_id){ 
        ...
        $bgimg1 = \Env::get('root_path').'public/static/images/share_bg.png';
        $image = \think\Image::open($bgimg1);  
        $image->water($qr,[255,743])->text($invite_code,\Env::get('root_path').'public/static/fz.TTF',22,'#ffffff',[(678-(24*strlen($user_id)))/2,685])->save(\Env::get('root_path').'public/upload/qrcode/user/'.$n.'/'.$user_id.'-1.png');
    }
    ...
    {
        $pid = db($this->table)->where('id',$uid)->value('parent_id');//上一级id
        $uinfo = db($this->table)->where('id',$pid)->find();//上一级信息
        if($uinfo){
